...
- **Catalogue exhaustif de droits** (32 fonctionnalités en 8 catégories : limites, carnet de santé, suivi quotidien, reproduction et pedigree, rappels et agenda, annuaire et communauté, partage et données, professionnels). Chaque onglet, rubrique de menu, bouton d'ajout rapide, action et fenêtre de l'app est rattaché à un code via la table `ROUTE_FEATURES` (app.js) ; le serveur applique `cloud_sync`, `vet_share`, `acym_lookup`, `email_reminders`, `push_reminders`, `monthly_summary` et les quotas. L'export de ses propres données n'est volontairement pas verrouillable (portabilité). Migration `seed_features_v3` : l'ancien code `nutrition_activity_checkup` est éclaté en `nutrition_plan`, `activities` et `health_checkup` en conservant les droits existants.
